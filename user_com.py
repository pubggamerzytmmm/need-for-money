...
def init_lnguage(lang, user_id):
    conn = sqlite3.connect('base.db')
    cursor = conn.cursor()
    cursor.execute('UPDATE users SET language_=:language_ WHERE id=:id', {'language_': lang, 'id': user_id})
    conn.commit()
    cursor.close()
    conn.close()
...
def select_language(user_id):
    conn = sqlite3.connect('base.db')
    cursor = conn.cursor()
    cursor.execute('SELECT language_ FROM users WHERE id=:id', {'id': user_id})
    l = cursor.fetchone()
    conn.commit()
    cursor.close()
    conn.close()
